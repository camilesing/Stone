...
import com.camile.ast.AbstractLeaf;
import com.camile.ast.AbstractList;
...
public class Parser {
    protected static abstract class Element {
        protected abstract void parse(Lexer lexer, List<AbstractTree> res)
    ...
    protected static class Repeat extends Element {
        ...
        protected void parse(Lexer lexer, List<AbstractTree> res) throws ParseException {
            while (parser.match(lexer)) {
                AbstractTree t = parser.parse(lexer);
                if (t.getClass() != AbstractList.class || t.numChildren() > 0) {
                    res.add(t);
                }
                if (onlyOnce) {
                    break;
    ...
    protected static abstract class AToken extends Element {
        ...
        protected AToken(Class<? extends AbstractLeaf> type) {
            if (type == null) {
                type = AbstractLeaf.class;
            }
        ...
        protected void parse(Lexer lexer, List<AbstractTree> res)
            ...
            Token t = lexer.read();
            if (test(t)) {
                AbstractTree leaf = factory.make(t);
                res.add(leaf);
            } else {
    ...
    protected static class IdToken extends AToken {
        ...
        protected IdToken(Class<? extends AbstractLeaf> type, HashSet<String> r) {
            super(type);
    ...
    protected static class NumToken extends AToken {
        protected NumToken(Class<? extends AbstractLeaf> type) {
            super(type);
        }
    ...
    protected static class StrToken extends AToken {
        protected StrToken(Class<? extends AbstractLeaf> type) {
            super(type);
        }
    ...
    protected static class Leaf extends Element {
        ...
        protected void find(List<AbstractTree> res, Token t) {
            res.add(new AbstractLeaf(t));
        }
    ...
    protected static class Expr extends Element {
        ...
        protected Expr(Class<? extends AbstractTree> clazz, Parser exp, Operators map) {
        ...
        @Override
        public void parse(Lexer lexer, List<AbstractTree> res) throws ParseException {
            AbstractTree right = factor.parse(lexer);
            Precedence prec;
        ...
        private AbstractTree doShift(Lexer lexer, AbstractTree left, int prec)
                throws ParseException {
            ArrayList<AbstractTree> list = new ArrayList<AbstractTree>();
            list.add(left);
            list.add(new AbstractLeaf(lexer.read()));
            AbstractTree right = factor.parse(lexer);
            Precedence next;
    ...
    protected static abstract class Factory {
        protected abstract AbstractTree make0(Object arg) throws Exception;

        protected AbstractTree make(Object arg) {
            try {
                return make0(arg);
        ...
        protected static Factory getForASTList(Class<? extends AbstractTree> clazz) {
            Factory f = get(clazz, List.class);
            if (f == null) {
                f = new Factory() {
                    @Override
                    protected AbstractTree make0(Object arg) throws Exception {
                        List<AbstractTree> results = (List<AbstractTree>) arg;
                        if (results.size() == 1) {
                            return results.get(0);
                        } else {
                            return new AbstractList(results);
                        }
                    }
                };
            }
        ...
        protected static Factory get(Class<? extends AbstractTree> clazz, Class<?> argType) {
            if (clazz == null) {
                return null;
            }
            try {
                ...
                return new Factory() {
                    @Override
                    protected AbstractTree make0(Object arg) throws Exception {
                        return (AbstractTree) m.invoke(null, arg);
                    }
                };
            } catch (NoSuchMethodException ignore) {
            }
            try {
                final Constructor<? extends AbstractTree> c = clazz.getConstructor(argType);
                return new Factory() {
                    @Override
                    protected AbstractTree make0(Object arg) throws Exception {
    ...
    public Parser(Class<? extends AbstractTree> clazz) {
        reset(clazz);
    }
    ...
    public AbstractTree parse(Lexer lexer) throws ParseException {
        ArrayList<AbstractTree> results = new ArrayList<AbstractTree>();
    ...
    public static Parser rule(Class<? extends AbstractTree> clazz) {
    ...
    public Parser reset(Class<? extends AbstractTree> clazz) {
        elements = new ArrayList<Element>();
        factory = Factory.getForASTList(clazz);
        return this;
    }
    ...
    public Parser number(Class<? extends AbstractLeaf> clazz) {
    ...
    public Parser identifier(Class<? extends AbstractLeaf> clazz, HashSet<String> reserved) {
    ...
    public Parser string(Class<? extends AbstractLeaf> clazz) {
    ...
    public Parser expression(Class<? extends AbstractTree> clazz, Parser subexp, Operators operators) {
